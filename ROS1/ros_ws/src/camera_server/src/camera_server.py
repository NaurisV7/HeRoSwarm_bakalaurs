...
class CameraServer():
        ...
        while True:
            if not image_queue.empty():
                ...
                if not self.display_queue.full():
                    self.display_queue.put_nowait(dimg1)
    ...
    def __init__(self):
        ...
        self.x_distance = 1.24
        self.y_distance = 0.74
        ...
        self.options.quad_decimate = 1.0
        self.options.refine_edges = 1
        ...
        _robots_json = os.path.join(os.path.dirname(os.path.abspath(__file__)), "robots.json")
        with open(_robots_json) as file:
...
if __name__ == '__main__':
        try:
            ...
            while not rospy.is_shutdown():
                if not server.display_queue.empty():
                    img = server.display_queue.get_nowait()
                    cv2.imshow("Camera Server", img)
                cv2.waitKey(1)
