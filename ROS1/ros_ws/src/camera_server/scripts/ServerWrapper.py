...
class ServerWrapper():
    ...
    def position_callback(self,msg,active_bots):
        for odom in msg.robot_pos:
            tag_id = odom.child_frame_id
            if tag_id not in self.robot_dictionary:
                continue
            name = self.robot_dictionary[tag_id]
            if name not in active_bots:
                continue
            x = odom.pose.pose.position.x
            y = odom.pose.pose.position.y
            theta = -self.rpy_from_quaternion(odom.pose.pose.orientation)[2]
            active_bots[name]["global_pos"] = [x, y, theta]
            active_bots[name]["global_pos_time"] = time.time()
    ...
    def stop(self):
        self.set_velocities([[0.0,0.0]]*len(self.get_data("global_pos")))
        self.step()
    ...
    def set_points(self,points):
        for (index,point) in enumerate(points):
            if not point == None:
                ...
                self.active_bots[index]["point"] = Point(point[0],point[1],0.0)
    ...
    def remove_bots(self):
        while True:
            for id in list(self.missing_bots.keys()):
                try:
                    t = self.missing_bots[id]
                    if t is not None and t < time.time() - self.timeout:
                        name = self.active_bots[id]["name"]
                        self.active_bots.pop(id, None)
                        self.active_bots.pop(name, None)
                        self.missing_bots.pop(id, None)
    ...
    def __init__(self,selected_bots=0) -> None:
        ...
        _robots_json = os.path.join(os.path.dirname(os.path.abspath(__file__)), "../src/robots.json")
        with open(_robots_json) as f:
            self.robot_dictionary = json.load(f)
